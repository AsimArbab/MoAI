...
def load_order():
	"""
        loads an order from a file and reads it out
    """
	global ordername
...
def write_order():
	"""
        reads in a typed order
		prints the final order out in the GUI
    """
	order_string = scr.get(1.0, 99.0)
	global print_order
	if len(order_string) != 1: 			# if something was typed in, treat it as order
		print_order = order_string
	else: 								# otherwise use the loaded order
		global read_order
		print_order = (', \n'.join(read_order.stock))
	
	placed_label.configure(text="Current Order: \n" + print_order) # prints out the current order in the GUI, while each time updating the label, instead of creating a new one
...
scrolH = 5 
scr = scrolledtext.ScrolledText(orderFrame, width=scrolW, height=scrolH, wrap=tk.WORD) # create scrolled text widget
scr.grid(column=1, sticky='WE', columnspan=2, padx=10, pady=5) # place widget
...
def solve():
	...
	if selected == True:	# if a method was selected
		...
		result = whouse_object.bring_item(order, method, n) # calculate the SOLUTION with the warehouse object, the current order, the chosen_method and the k (if existent)
		...
		psu_label.configure(text=''.join(psu_list)) # prints out the result in the GUI, while each time updating the label, instead of creating a new one
...
psu_label = ttk.Label(methodsFrame, text='')
psu_label.grid(column=4, sticky='W', row=8, padx=10, pady=5)
